  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 mt-1">All generated signals with risk decisions</p>
      </div>
      ...
      <div className="bg-surface-900 border border-gray-800 rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={9} className="p-8 text-center text-gray-500">Loading...</td>
              </tr>
            ) : signals.length === 0 ? (
              <tr>
              ...
              signals.map((sig) => (
                <tr key={sig.id} className="border-b border-gray-800/50 hover:bg-surface-800/50">
                  ...
                  <td className="p-4 font-mono-nums">₹{sig.entry_price?.toFixed(2) || '—'}</td>
                  <td className="p-4 font-mono-nums text-red-400">₹{sig.stop_loss?.toFixed(2) || '—'}</td>
                  <td className="p-4 font-mono-nums text-emerald-400">₹{sig.target_price?.toFixed(2) || '—'}</td>
                  <td className="p-4 font-mono-nums">{sig.quantity || '—'}</td>
                  <td className="p-4 font-mono-nums">{sig.risk_reward?.toFixed(1) || '—'}</td>
                  <td className="p-4">
                    ...
                    {sig.block_reason && (
                      <p className="text-xs text-gray-500 mt-1">{sig.block_reason}</p>
                    )}
                  </td>
                </tr>
              ))
